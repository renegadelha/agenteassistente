...
if not GROQ_KEY:
    raise ValueError("Atenção: A chave da API do Groq não foi encontrada!")
    ...
    with mic as source:
        ...
        while True:
            ...
            try:
                ...
                print("[JARVIS]: Processando sua voz via Groq...")

                # Salva o áudio temporariamente em WAV
                caminho_temp = "temp_voz.wav"
                with open(caminho_temp, "wb") as f:
                    f.write(audio_gravado.get_wav_data())

                # Envia o arquivo para a API ultrarrápida do Groq
                with open(caminho_temp, "rb") as file:
                    texto = groq_client.audio.transcriptions.create(
                        file=(caminho_temp, file.read()),
                ...
                print("\n" + "=" * 40)
                print(f"🗣️ VOCÊ DISSE: '{texto.strip()}'")
                print("=" * 40 + "\n")
